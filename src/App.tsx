...
function GoalModule(){const {goals,addGoal,updateGoal,delGoal}=useStore(); const done=goals.filter(g=>g.done).length;
 ...
 return <div className='card space-y-2'><div>{done}/{goals.length}</div><div className='grid md:grid-cols-2 gap-2'>{goals.map(g=><div key={g.id} className='bg-slate-800 p-2 rounded'><h3>{g.title}</h3><p>{g.desc}</p>{g.image&&<img src={g.image} className='h-24 object-cover'/>}<div className='flex gap-2'><button onClick={()=>updateGoal(g.id,{done:!g.done})}>{g.done?'달성됨':'진행중'}</button><button onClick={()=>delGoal(g.id)}>삭제</button></div></div>)}</div><button className='btn' onClick={()=>{const title=prompt('제목'); if(!title) return; const desc=prompt('설명')||''; addGoal({id:id(),title,desc,done:false})}}>목표 추가</button><input type='file' onChange={(e)=>{const f=e.target.files?.[0]; if(!f||!goals.at(-1)) return; onFile(f,(img)=>updateGoal(goals.at(-1)!.id,{image:img}))}}/></div>
}
